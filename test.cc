...
int main()
{
	auto const my_null_cache = new Cache(100,0.75,nullptr, std::hash<key_type>());
	//Note: Your fifo_evictor class might have a different name
	auto const my_fifo_cache = new Cache(150, 0.75, new Fifo_evictor(), std::hash<key_type>());
	auto const my_null_evictor_collide = new Cache(100, 0.75, nullptr, liam_hash);
	//Basic test
	std::cout << "---------------------------------basic test started---------------------------------------" << '\n' << '\n';
	basic_test(my_null_cache);
	...
	null_evictor_test(my_null_cache);
	std::cout << "null evictor test passed!" << '\n';

	//we test for deep copy last 
	std::cout << "---------------------------------deep copy test started---------------------------------------" << '\n' << '\n';
	deep_copy_test(my_null_cache);
	std::cout << "deep copy test passed!" << '\n';
	delete my_null_cache;

	//Test for collision
	std::cout << "---------------------------------collision test started---------------------------------------" << '\n' << '\n';
	Collision_test(my_null_evictor_collide);
	std::cout << "collision test passed!" << '\n';
	delete my_null_evictor_collide;


	//Test for fifo_evictor
	std::cout << "---------------------------------fifo evictor test started---------------------------------------" << '\n' << '\n';
	fifo_evictor_test(my_fifo_cache);
	std::cout << "fifo evictor test passed!" << '\n';
	delete my_fifo_cache;
	
	//We initially wrote a test for resizing. However, that only works for our implementation and we figured there was not a way to test resize without knowing the initial hashtable size
